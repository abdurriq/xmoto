...
int UploadAllHighscoresThread::realThreadFunction() {
  ...
  try {
    ...
    std::string webRoomUrl =
      GameApp::instance()->getWebRoomURL(m_number, m_pDb);
  ...
  std::string query =
    "SELECT r.id_level, r.name, lvl.name FROM replays r "
    "LEFT OUTER JOIN webhighscores h "
    "ON (r.id_level = h.id_level AND h.id_room=" +
    ...
    "\" "
    "AND r.isFinished=1 "
    "AND ( (h.id_room IS NULL) OR xm_floord(h.finishTime*100.0) > "
    "xm_floord(r.finishTime*100.0)) "
